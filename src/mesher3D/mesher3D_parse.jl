...
function parsenodes(fileinstance::IOStream, all_entities_struct::All_entities_struct)::Dict{Int64, Nodestruct}
    ...
    for _ in 1:num_node_entities
        ...
        # find which other entities each node belongs to transitively
        # e.g. node belongs to root_entitydim, and if root_entitydim in turn belongs to entity x
        # then by transitivity, node belongs to entity x
